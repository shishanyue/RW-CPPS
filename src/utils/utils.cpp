...
int32_t RandomUtils::randomInt32(int32_t origin, int32_t bound) {
    //随机数范围
    uniform_int_distribution<int32_t> distribution(origin,bound);
    return distribution(*this->randomEngine);
}

string RandomUtils::randomBigLetter(size_t _n) {
    string str;
    for(int i = 0;i<=_n;i++){
        str+=static_cast<char>(randomInt32(65,90));
    }
    return str;
}
...
bool StringUtils::startsWith(string *str, const char *c_str) {
    size_t c_strLength = strlen(c_str);
    if(str->length() >= c_strLength){
        string s = str->substr(0,c_strLength);
        if(str->substr(0,c_strLength) == c_str){
            str->erase(0,c_strLength);
            return true;
        }return false;
    }else return false;
}

bool StringUtils::scanUnsignedInteger(const char **str) {
...
bool StringUtils::scanInteger(const char **str) {
    if (**str == '+' || **str == '-')
        ++(*str);
...
MD5::MD5(const string& message) {
    ...
    init((const mdbyte*)message.c_str(), message.length());
}
 ...
 *
 */
void MD5::init(const mdbyte* input, size_t len) {
...
void MD5::transform(const mdbyte block[64]) {
...
*
*/
void MD5::encode(const bit32* input, mdbyte* output, size_t length) {
 ...
 *
 */
void MD5::decode(const mdbyte* input, bit32* output, size_t length) {
    for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
 ...
 */
string MD5::toStr() {
    const mdbyte* digest_ = getDigest();
    string str;
    str.reserve(16 << 1);
